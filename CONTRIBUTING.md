Contributing to ArtBytecore
=======

We're working hard to make *artbytecore* the most powerful JavaScript library for working with artbyte. Our goal is to have *artbytecore* be a library that can be used by anyone interested in artbyte, and to level expertise differences with great design and documentation.
...
These are some global design goals in artbytecore that any change must adhere.
...
When possible, artbytecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of ArtBytecore methods. For example, calling `JSON.stringify` on an Bitcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'artbytecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the artbytecore object.

DO:
```javascript
var artbytecore = require('../');
var PublicKey = artbytecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add bushstar git@github.com:Bushstar/artbytecore.git
git pull --rebase bushstar master
```
...
```
Finally go to [github.com/Bushstar/artbytecore](https://github.com/Bushstar/artbytecore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of artbytecore.

If you have any questions feel free to post them to
[github.com/Bushstar/artbytecore/issues](https://github.com/Bushstar/artbytecore/issues).
