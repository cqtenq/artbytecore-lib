...
Transaction.MAX_MONEY = 313000000 * 1e8;
 ...
 *
 * Can receive, as output information, the output of artbyte-cli's `listunspent` command,
 * and a slightly fancier format recognized by artbytecore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a artbytecore Address object. The
 * same is true for `script`, which can be a string or a artbytecore Script.
 *
